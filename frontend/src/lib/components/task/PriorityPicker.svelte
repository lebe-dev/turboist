<script lang="ts">
	...
	let {
		value = $bindable<Priority>('no-priority'),
		disabled = false
	}: { value?: Priority; disabled?: boolean } = $props();
</script>

{#if disabled}
	<span
		class="inline-flex h-8 cursor-not-allowed items-center gap-1.5 rounded-md border border-border bg-muted/30 px-2.5 text-xs font-medium text-muted-foreground opacity-80"
		aria-label="Priority (locked)"
	>
		<FlagIcon
