...
func (s *BackupService) readTasks(ctx context.Context) ([]BackupTask, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id, title, description, inbox_id, context_id, project_id, section_id, parent_id,
				...
				source_task_id, auto_sorted_at, auto_sort_undecided_at, created_at, updated_at
	...
	for rows.Next() {
		...
		var dueAt, deadlineAt, pinnedAt, recurrence, completedAt, troiki, autoSortedAt, autoSortUndecidedAt sql.NullString
		...
		if err := rows.Scan(&t.ID, &t.Title, &t.Description,
			...
			&sourceTaskID, &autoSortedAt, &autoSortUndecidedAt, &t.CreatedAt, &t.UpdatedAt); err != nil {
			return nil, err
		}
		...
		if autoSortUndecidedAt.Valid {
			v := autoSortUndecidedAt.String
			t.AutoSortUndecidedAt = &v
		}
