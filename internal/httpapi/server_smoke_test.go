...
func smokeApp(t *testing.T) *smokeEnv {
	...
	handlers.NewTaskBulkHandler(completeSvc, moveSvc, groupSvc, taskSvc, baseURL).Register(api)
	...
	handlers.NewMetaHandler(cfg, false, ctxRepo, projectRepo, labelRepo, taskRepo, userRepo, appSettingsRepo, troikiSvc, baseURL).Register(api)
