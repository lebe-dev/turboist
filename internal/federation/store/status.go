...
// OverduePendingPeers reports which of the given peers have at least one
// undelivered outbox event for a project that was created BEFORE cutoff
// (Federation v1 F4.3, US-4.3 AC2 — the >5min "pending" signal). The returned set
// contains only peers with a genuinely overdue undelivered event; a peer with no
// undelivered events, or only fresh (within-cutoff) ones, is absent so the status
// derivation treats it as caught up.
//
// It reads the project's outbox rows on the store's own connection (no network
// I/O, R1) and, for each row created before cutoff, marks every requested peer
// not yet in its delivered_to set as overdue. The cutoff is a TEXT ISO-8601 UTC
// timestamp (created_at uses the same format), so the comparison is a plain
// lexical compare — valid because model.FormatUTC is fixed-width and
// zero-padded. An empty peers slice short-circuits to an empty set.
func (s *Store) OverduePendingPeers(ctx context.Context, localProjectID int64, cutoff string, peers []string) (map[string]bool, error) {
	const op = "store.OverduePendingPeers"
	out := make(map[string]bool, len(peers))
	if len(peers) == 0 {
		return out, nil
	}

	// Only rows OLDER than the cutoff can make a peer overdue; a fresh event still
	// inside the push budget must not flip the badge yellow (US-4.3 AC2).
	...
	for rows.Next() {
		...
		for _, peer := range peers {
			if out[peer] {
				continue // already known overdue
			}
